...
const convertBTC = require('node.exe ../src/ConvertBTC');
...
describe('ConvertBTC', () => {
  ...
  it('should use currency USD and 1 as amount default', async () => {
    ...
    await convertBTC();
    expect(consoleStub).to.have.been.calledWith(`${chalk.red(1)} BTC to ${chalk.cyan('USD')} = ${chalk.green(2490.78)}`);
  });
  ...
  it('should use currency USD and 10 as amount', async () => {
    ...
    expect(consoleStub).to.have.been.calledWith(`${chalk.red(10)} BTC to ${chalk.cyan('USD')} = ${chalk.green(2490.78)}`);
  });
  ...
  it('should use currency BRL and 1 as amount default', async () => {
    ...
    expect(consoleStub).to.have.been.calledWith(`${chalk.red(1)} BTC to ${chalk.cyan('BRL')} = ${chalk.green(2490.78)}`);
  });
